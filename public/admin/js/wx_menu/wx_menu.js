...
$(function() {
	...
	$("#pubBt").click(function(){
		if(validation()){
			$.ajax({
				url : __URL(ADMINMAIN+"/wchat/updatemenutoweixin"),
				type : "post",
...
//			showDialog("请先按要求编辑完当前操作！！！", 0);
	 ...
	 * 预览文章
	 */
	$(".js-preview").live("click",function(){
		if($(this).attr("data-media-item-id")) window.open(__URL(APPMAIN + "/wchat/templateMessage?media_id=" + $(this).attr("data-media-item-id")));
	});
...
function clearMediaMessage(){
	...
	$("#show_media .js-preview").removeAttr("data-media-item-id");
}
...
function updateWeixinMenuEventType(menu_id,menu_event_type){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/updateweixinmenueventtype"),
		type : "post",
		async : false,
...
function addWeixinMenu(menu){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/addweixinmenu"),
		type : "post",
		async : false,
...
function updateWeixinMenuName(menu_id, menu_name){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/updateweixinmenuname"),
		type : "post",
		async : false,
...
function updateWeixinMenuUrl(menu_id,menu_event_url){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/updateweixinmenuurl"),
		type : "post",
		async : false,
...
function deleteWeixinMenu(menu_id){
	...
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/deleteweixinmenu"),
		type : "post",
		async : false,
...
function updateWeiXinMenuMessage(menu_id, media_id, menu_event_type){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/updateweixinmenumessage"),
		type : "post",
		async : false,
	...
	if(mediaObj.type == 2 || mediaObj.type == 3){
		...
		for(var i=0;i<list.length;i++){
			if(i == 0){
				...
				var style = "background-image:url('" + __IMG(list[i]["cover"]) + "')";
				$(".cover_appmsg_item>div").attr("style",style);
				$(".cover_appmsg_item>div").text("");
				$(".cover_appmsg_item>.js-preview").attr("data-media-item-id",list[i].id);
			}else{
				html += '<div class="appmsg_item has_cover">';
				html += '<div class="appmsg_thumb_wrp" style="background-image:url(' + __IMG(list[i]["cover"]) + ');"></div>';
				html += '<h4 class="appmsg_title js_title"><a href="' + list[i]["content_source_url"] + '" target="_blank">' + list[i]["title"] + '</a></h4>';
				//预览文章地址,showDialog(&#39;微信还没有给您生成呢，想啥呢？不能预览！！！&#39;)
				html += '<a href="javascript:;" class="edit_mask preview_mask js-preview" data-media-item-id="' + list[i].id + '">';
		...
		$(".cover_appmsg_item>.js-preview").attr("data-media-item-id",mediaObj.item_list[0].id);
	}
...
function updateWeixinMenuSort(menu_id_arr,flag){
	$.ajax({
		url : __URL(ADMINMAIN + "/wchat/updateweixinmenuSort"),
		type : "post",
		async : false,
...
//	console.log(str.indexOf("http") != -1 || str.indexOf("https") != -1);
